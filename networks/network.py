...
"""
以resnet32为例:
    1.remove_existing_head
        remove_existing_head决定是否扔掉最后一个fc层( linear(64,10) ),
        扔掉后用的就是完全展平的特征(64维)，不扔掉就是用一个fc层变换后的特征(10维)，
        还是扔掉比较好，甚至应该改成输出特征维是256
    
    2。add_head函数:
        此函数是和task-IL一起写的，由于Class IL没有task-id，所以就把最后的head的输出y (一个list)全concat起来作为输出；
        head相当于就是最后的分类器，每一个head负责一个experience的分类， 
"""
class LLL_Net(nn.Module):
    ...
    def __init__(self, model, remove_existing_head=False):
        ...
        self.model = model
        last_layer = getattr(self.model, head_var) # getattr(x,y) equ to x.y
        ...
        if remove_existing_head:
            if type(last_layer) == nn.Sequential:
                ...
                setattr(self.model, head_var, nn.Sequential()) # setattr(x,y,v) equ to x.y=v
        else:
    ...
    def add_head(self, num_outputs):
        ...
        # if len(self.heads) > 1:
        #     self.heads[-1].weight.data = self.heads[-2].weight.data #initilization
    ...
    def forward(self, x, return_features=False):
        ...
        for head in self.heads:
            y.append(head(x)) #class IL 这种无task-id的就直接把y全部concatenate起来
        if return_features:
            return y, x
        else:
            return y
